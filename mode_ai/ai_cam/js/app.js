(function () {
  ...
  const CODE_MAP = { rock: '▓▓', scissors: '◈◈', paper: '░░' };
  ...
  async function initializeCamera() {
    ...
    // Training LSTM
    try {
      if (textInfo) textInfo.textContent = 'MELATIH AI...';
      lstmModel = new LSTMPredictor();
      await lstmModel.train(function (epoch, total) {
        if (textInfo) textInfo.textContent = 'MELATIH AI... ' + Math.round((epoch / total) * 100) + '%';
      });
      isLSTMReady = true;
      if (textInfo) textInfo.textContent = 'AI SIAP, KELUARKAN TANGANMU 👋';
    } catch (errorMsg) {
      if (textInfo) textInfo.textContent = 'SIAP (AI mode random)';
      isLSTMReady = false;
    }
      ...
      if (trackResult && trackResult.multiHandLandmarks && trackResult.multiHandLandmarks.length) {
        if (!hasGuessed) {
          ...
          if (guessResult && guessResult.confidence >= 0.5) {
            ...
            let choiceInt = CHOICE_TO_INT[playerGuess];
        ...
        hasGuessed = false;
        if (textInfo) textInfo.textContent = isLSTMReady ? 'AI SIAP, KELUARKAN TANGANMU 👋' : 'SIAP, KELUARKAN TANGANMU';
        removeHighlights();
